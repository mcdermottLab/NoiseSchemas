...
timing_analysis_file = fullfile(file_path, sprintf('../../results/%s/spectrotemporal_split_timing_analysis.mat', exp_name)); % get path to save timing analysis
...
niters = 10000;
est_params_high = nan(niters,3); % iteration by params (slope, int, elbow point)
est_params_low = nan(niters,3); % iteration by params (slope, int, elbow point)
rng(1)
parfor n = 1:niters
    p = randi(n_subs,n_subs,1);

    y = mean(d_primes_high(:,p),2,'omitnan')';
    est_params = fit_elbow_func(x,y);  
    est_params_high(n,:) = est_params;

    y = mean(d_primes_low(:,p),2,'omitnan')';
    est_params = fit_elbow_func(x,y);  
    est_params_low(n,:) = est_params;
end
toc
save(timing_analysis_file, 'est_params_high', 'est_params_low')
