...
% calculate d prime for train, val, and test trials
...
for i = 1:length(results)
    ...
    responses_to_use = responses; % to work with compute_subject_rates
    responses_to_use(sum(responses_to_use,2)==0,:) = []; % remove trials with no data                           
    responses_to_use(:, 2) = 1;
    ...
    d_primes(ID2, i) = norminv(hit_rates) - norminv(mean_false_alarm_rate);

end
d_primes = d_primes(:, logical(keep_subs));
d_primes = mean(cat(3, d_primes(1:10,:), d_primes(11:20,:)),3, 'omitnan'); % average over SNR
...
for i = 1:length(results)    
    responses = results(i).responses;
    responses = responses(responses(:,1) < 900,:); % remove catch trials
    keep_subs(i) = results(i).pass;
    ...
    hit_rates = splitapply(@compute_hit_rate, responses_to_use(:, 4), responses_to_use(:, 5), G);  % get hit rates and false alarm rates for all conditions (SNR by onset time)      
    mean_false_alarm_rate = compute_false_alarm_rate(responses_to_use(:, 4), responses_to_use(:, 5)); % compute false alarms over all "conditions" since they only apply with foregrounds present    
    d_primes_comparison1(ID2, i) = norminv(hit_rates) - norminv(mean_false_alarm_rate);
...
for i = 1:length(results)    
    ...
    responses_to_use = responses(:, 2:end); % to work with compute_subject_rates
    responses_to_use(sum(responses_to_use,2)==0,:) = []; % remove trials with no data                           
    responses_to_use(:, 2) = 1;   
    [G, ID1, ID2] = findgroups(responses_to_use(:,2),responses_to_use(:,3)); % get the group info
    group_info = [ID1 ID2];
    hit_rates = splitapply(@compute_hit_rate, responses_to_use(:, 4), responses_to_use(:, 5), G);  % get hit rates and false alarm rates for all conditions (SNR by onset time)      
    mean_false_alarm_rate = compute_false_alarm_rate(responses_to_use(:, 4), responses_to_use(:, 5)); % compute false alarms over all "conditions" since they only apply with foregrounds present    
    d_primes_comparison7(ID2, i) = norminv(hit_rates) - norminv(mean_false_alarm_rate);
end
