...
wav_exp_name = 'exp6';
wav_path = fullfile(file_path, sprintf('../../figures/%s/wavs/', wav_exp_name)); % get path to raw waveforms
results_file = fullfile(file_path, sprintf('../../results/%s/results.mat', exp_name)); % get path to results
load(results_file)
matched_permutation_test_file = fullfile(file_path, sprintf('../../results/%s/matched_permutation_test_%s.mat', exp_name, exp_name)); % get path to results
...
rep_colors = [192 9 19; 152 9 15; 217 47 49]/255;
...
s = std(d_primes_rep,0,2)/sqrt(n_subs);
ub = m + s;
lb = m - s;
...
s = std(d_primes_nonrep,0,2)/sqrt(n_subs);
ub = m + s;
lb = m - s;
x_patch = [timevec, flip(timevec)];
y_patch = [ub(:,1)', flip(lb(:,1)')];
...
[l, objh]  = legend({'  Repeated', '  Non-Repeated'},'Location','northeast');
...
l.Position(1) = l.Position(1)*0.95;
