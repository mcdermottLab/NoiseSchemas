...
for f = 1:length(files)
    ...
    elseif strcmp(data_table.Q6hearingloss{:}, 'yesHL') % do not include if they self-report hearing loss
        ...
        if sum(sum(responses, 2)==0) <= round(0.1 * n_trials) % subject is missing at most 10% of trials
            ...
            group_info = [ID1 ID2];
            hit_rates = splitapply(@compute_hit_rate, responses_to_use(:, 4), responses_to_use(:, 5), G);  % get hit rates and false alarm rates for all conditions (interrupter type by onset time)      
            mean_false_alarm_rate = splitapply(@compute_false_alarm_rate, responses_to_use(:, 4), responses_to_use(:, 5), responses_to_use(:,2)); % compute false alarms over interrupter types
