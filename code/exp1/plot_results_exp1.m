...
% set colors
main_color = [34, 113, 178]; % "honolulu blue" see http://mkweb.bcgsc.ca/colorblind/palettes.mhtml (Martin Krzywinski — Designing for Color Blindness) 
...
outliers = [1 2 2]; % presence of outlier
...
text(length(background)/2, y_positions(1) + clip_thresh*2.1, sprintf('Did you hear one or two sounds?'), ...
...
% add SNR labels
text(timevec(end), max(ub(9:10,1))+0.08, sprintf('%d dB', SNRs(1)), ...
     'FontSize', 25, ...
     'FontAngle','italic', ...
     'Color',clrs(1,:), ...
     'HorizontalAlignment','right')
text(timevec(end), max(ub(9:10,2))+0.08, sprintf('%d dB', SNRs(2)), ...
     'FontSize', 25, ...
     'FontAngle','italic', ...
     'Color',clrs(2,:), ...
     'HorizontalAlignment','right')
