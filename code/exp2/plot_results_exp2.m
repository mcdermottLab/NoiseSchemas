...
% set colors
main_color = [61, 183, 233]; % "summer sky" see http://mkweb.bcgsc.ca/colorblind/palettes.mhtml (Martin Krzywinski — Designing for Color Blindness) 
...
nstims = 3;
stims = [123 99 22]; % stim IDs (bg/fg pairing)
outliers = [2 2 2]; % presence of outlier
times = [3 1 7]; % foreground onset times
snrs = [2 1 1]; % SNRs
trials = [1 2 320]; % trial numbers
...
skip_position = y_positions(end-1); % position that is skipped and replaced with ellipsis
y_positions(end-1) = []; % actually remove the skipped position
% make a bit tighter
skip_position = skip_position + clip_thresh*0.75;
y_positions(end) = y_positions(end) + clip_thresh*1.5;
...
for n = 1:nstims
    ...
    text(-0.25*audio_sr + x_offset, y_positions(n), sprintf('%d', trials(n)), 'HorizontalAlignment','center', 'FontSize', 24, 'VerticalAlignment','middle')
...
text(-0.25*audio_sr + 0.025*audio_sr, y_positions(1) + clip_thresh*1.25, 'trial', 'HorizontalAlignment','center', 'FontSize', 24, 'VerticalAlignment','middle')
text(length(background)/2, y_positions(1) + clip_thresh*1.25, 'stimulus', 'HorizontalAlignment','center', 'FontSize', 24, 'VerticalAlignment','middle')
text(length(background)/2, y_positions(1) + clip_thresh*2.1, sprintf('Which sound did you hear?'), ...
    'HorizontalAlignment','center', 'FontSize', 26, 'VerticalAlignment','middle', 'FontWeight','bold', 'FontAngle','italic', 'Color', clrs(1,:))

% save diagram figure
set(gca,'LooseInset',get(gca,'TightInset')); % remove space around edges
ax = gca;
ax.Position(1) = ax.Position(1)*0.5;
ax.Position(2) = -0.12;
ax.Position(4) = 1.5;
...
m = reshape(mean(percent_correct,2),10,2);
s = reshape(std(percent_correct,0,2),10,2)/sqrt(n_subs);
ub = m + s;
lb = m - s;
...
% plot human results
scatter(timevec, mean(percent_correct(1:10,:),2), 125, 'MarkerEdgeColor', 'w', 'MarkerFaceColor', clrs(1,:), 'LineWidth', 0.75)
scatter(timevec, mean(percent_correct(11:20,:),2), 125, 'MarkerEdgeColor','w', 'MarkerFaceColor', clrs(2,:), 'LineWidth', 0.75)

% add SNR labels
text(timevec(end), max(ub(9:10,1))+0.02, sprintf('%d dB', SNRs(1)), ...
     'FontSize', 25, ...
     'FontAngle','italic', ...
     'Color',clrs(1,:), ...
     'HorizontalAlignment','right')
text(timevec(end), max(ub(9:10,2))+0.02, sprintf('%d dB', SNRs(2)), ...
     'FontSize', 25, ...
     'FontAngle','italic', ...
     'Color',clrs(2,:), ...
     'HorizontalAlignment','right')
...
% plot dividing line
plot([0 2750], [0.2, 0.2], 'k', 'LineWidth', 2)

% add number of subjects
text(75, 0.78, sprintf('n = %d', n_subs), ...
     'FontSize', 30, ...
     'FontAngle','italic', ...
     'Color',ones(3,1)*0)
...
box off
ylabel('Recognition Performance (% Correct)', 'Position', [-300, .5])
xlabel('Foreground Onset Time (ms)')
ylim([0.15 0.8])
yticks(0.2:0.1:0.8)
yticklabels(20:10:80)
xlim([0 2750])
xticks((timevec))
set(gca,'linewidth',2)
